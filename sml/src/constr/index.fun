      ...
      fn (D {consIndex, antsIndex, ...}, seq) =>
         let
            ...
            fun foldFn (a_rel, cands) =
               S.intersection (Index.instances (antsIndex, a_rel), cands)
            ...
            case Cons.show (Seq.cons seq) of
               Cons.Xi => let in
                  case Ants.hd ants of
                   ...
                   | SOME rel =>
                     let
                        val base = Index.instances (antsIndex, rel)
                     in
                        antsInter base
                     end
               end
             | Cons.P crel =>
               let
                  val cinsts = Index.instances (consIndex, crel)
               in
                  case Ants.hd ants of
                     NONE => cinsts
                   | SOME rel =>
   ...
   *)
   fun general (D {antsIndex, consIndex, varIndex, ...}, seq) =
      case Cons.show (Seq.cons seq) of
         ...
         Index.general (consIndex, crel)
       | Cons.Xi =>
         let
            val antsCandidates =
               let
                  fun foldFn (a_rel, cands) =
                     S.union (Index.general (antsIndex, a_rel), cands)
               in
      ...
      let
         ...
         fun existsFn id =
            let
               ...
               val q = getSeq (db, id)
            in
               isSome (Seq.subsumes { subsumer = q
                                    , subsumed = seq
                                    , global = global
                                    , entails = entails})
            end
   ...
   fun unifiable (D {antsIndex, consIndex, varIndex, ...}, seq) =
      let
         fun candFn (a_rel, cands) =
            S.union (Index.unifiable (antsIndex, a_rel), cands)
         val antsCandidates = Ants.fold candFn S.empty (Seq.ants seq)
      in
         case Cons.show (Seq.cons seq) of
            Cons.P crel =>
            S.union ( Index.unifiable (consIndex, crel),
                     S.intersection (VarIndex.set varIndex, antsCandidates))
          | Cons.Xi =>

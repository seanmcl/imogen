...
structure Timing :> Timing = struct
   ...
   (* user and system time add up to total CPU time used gc time is a
      portion of the total CPU time devoted to garbage collection *)
   datatype cpuTime = Cpu of
      { usr: Time.time
      , sys: Time.time
      , gc: Time.time }
   ...
   fun pp' (name, (CPUTime as Cpu {usr, sys, gc}, realTime)) =
      &[ PP.bracket ($name)
       , %[\\, &[ %[$"Real  : ", PP.time realTime]
                , %[ $"Run   : ", PP.time (sum CPUTime), $" (usr : "
                   , PP.time usr, $", sys : ", PP.time sys, $", gc  : "
                   , PP.time gc, $")"]
                ]
          ]
       ]

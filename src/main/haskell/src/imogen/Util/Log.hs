...
module Imogen.Util.Log
  ( module Imogen.Util.Log.Class
  , initialize
--  , stdout
  , logFileName
  , readPrio
...
-- Imogen starts logging with the default priority.
...
initialize prio file term = 
  ...
  do -- Erase the default stderr handler.
     ...
     M.when file $ do 
       ...
       M.when logExists $ Directory.removeFile logFileName 
       -- Open new log file

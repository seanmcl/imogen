...
structure Stream :> Stream = struct
   ...
   withtype 'a t = unit -> 'a front Lazy.t

   fun force s = Lazy.force (s ())
   ...
   and map' f s () = case force s of
      Nil => Nil
    | Cons (x,xs) => Cons (f x, map f xs)

   local
      fun mapj f n s = delay (mapj' f n s)
      and mapj' f n s () = case force s of
         Nil => Nil
   ...
   and append' (s1,s2) () = case force s1 of
      Nil => force s2
   ...
   fun all p s = case force s of
      Nil => true
    | Cons (x,xs) =>
      if not (p x) then false
      else all p xs

   fun exists p s = case force s of
      Nil => false
    | Cons (x,xs) =>
      if p x then true
      else exists p xs
   ...
   and filter' f s () = case force s of
      Nil => Nil
    | Cons (x, xs) =>
   ...
   and take' (_,0) () = Nil
     | take' (s,n) () = case force s of
                           Nil => Nil
                         | Cons (x,xs) => Cons (x,take (xs,n-1))

   fun drop (s,0) = s
     | drop (s,n) = case force s of
                       Nil => s
                     | Cons (_,xs) => drop (xs,n-1)

   fun ofList [] = empty
     | ofList (h::t) = delay (fn () => Cons (h,ofList t))

   local
      fun length' s acc = case force s of
         Nil => acc
   ...
   local
      ...
      and init1' y s () = case force s of
         Nil => Nil
       | Cons (z,zs) => Cons (y,init1 z zs)
   in
      fun init s = delay (init' s)
      and init' s () = case force s of
         Nil => raise Empty
   ...
   fun lexOrder f (s,t) = case (force s, force t) of
      (Nil,Nil) => EQUAL
    | (Nil,_) => LESS
    | (_,Nil) => GREATER
    | (Cons (x,xs),Cons (y,ys)) =>
   ...
   local
      fun index' p s n = case force s of
         Nil => NONE
       | Cons (x,xs) =>
         if p x then SOME n
         else index' p xs (n+1)

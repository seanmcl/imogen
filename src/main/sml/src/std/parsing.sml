...
structure Parsing :> Parsing = struct
   ...
   fun fail (pos,_) = raise Fail pos

   fun done x (pos,ts) = case Stream.force ts of
      Stream.Nil => (x, pos, pos, ts)
    | Stream.Cons _ => raise Fail (Pos.rightEdge pos)

   fun any (pos,ts) = case Stream.force ts of
   ...
   fun flat6 (a,(b,(c,(d,(e,f))))) = (a,b,c,d,e,f)
   ...
   structure Ops =
      ...
      val op wth = op wth
      val op when = op when
      val op return = op return
      val op suchthat = op suchthat
   end

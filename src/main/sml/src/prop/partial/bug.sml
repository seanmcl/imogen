...
structure Bug = struct
   ...
   val bugs = "(p => q) => p => q"
   val bugs = "p :: true"
   val bugs = "(x :: p) -> (y :: q) -> p & q"
   val bugs = "(x1 :: p -> q) -> (y1 :: p) -> q"
   val bugs = "(x1 :: p) -> (y1 :: q) -> q & p"
   val bugs = "(x1 :: p) -> (y1 :: q) -> q & q"

   (* val p1 = PP.hang ($"hi") 2 ($"sean") *)
   (* val p2 = PP.hang p1 2 p1 *)
   (* val p3 = PP.hang p2 2 p2 *)

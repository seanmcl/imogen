...
structure Backend :> Backend = struct
   ...
   fun addKeptSeq ( db as T {pdb, ksd, goal, foundGoal, conflicts, global, ...}
                  , seq, node:Node.t) =
               ...
               fun afn return (seq, theta) =
                  if subsumes (db, seq) then
                     ...
                     in
                      ...
                      ; if Seq.subsumes (seq, goal, {global=NONE}) then
                           let in
   ...
   fun addKeptRule (db as T {pdb, krd, goal, conflicts, ...}, rule, node) =
      ...
      in
         ...
         else if not (Conflicts.allowed (conflicts, Seq.preds concl)) then
   ...
   (* Step the database. *)
   fun step (db as T { goal, ksd, asd, ard, krd, pdb, iters, global, ...}) =
      let in
         Ref.incr iters
       ; case phase db of
            Done => S.Saturated
          | Seq =>
            ...
            in
               PDB.seq (pdb, qid, node)
             ; case Seq.subsumes' (seq, goal, {global=NONE}) of
                  SOME theta =>
                  let
                     val pf =
                  ...
                  let
                     ...
                     fun generate (rid, acc as (newQ, newR)) =
                        let
                           ...
                           val match =
                              Rule.match {rule = rule, seq = seq, global = global}
                           fun node (qr, theta) =
                              (qr, Node.Derived
            ...
            let
               ...
               fun generate (qid, acc as (newQ, newR)) =
                  let
                     ...
                     val (seq, renameQ) = Seq.rename (seq, Subst.id)
                     val (rule, renameR) = Rule.rename rule
                     val match = Rule.match {rule = rule, seq = seq, global = global}
                     val _ = Log.debug (fn () =>
                        &[ $"Rule.match:"
                         , %[\\, &[ &[$"Rule:", %[\\, Rule.pp rule]]
                                  , &[$"Seq:", %[\\, Seq.pp seq]]
